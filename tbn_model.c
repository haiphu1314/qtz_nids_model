...
TBN_Layer* tbn_read_model(const char* filename, int* num_layers) {
    ...
    TBN_Layer *layers = (TBN_Layer *)malloc(max_layers * sizeof(TBN_Layer));

    while (fgets(line, sizeof(line), file)) {
        if (strstr(line, "linear")) {
            TBN_Layer *layer = &layers[layer_count++];
    ...
    for (int l = 0; l < num_layers; ++l) {
        ...
        for (int i = 0; i < layer->output_channel; ++i) {
            ...
            for (int j = 0; j < sizeint_curr_input; ++j) {
                int weight = layer->weights[i*sizeint_curr_input+j];
                int i_weight = ~weight;
                int result_bit0 = (curr_input[j].bit_1 & i_weight) | (curr_input[j].bit_0 & weight);
                int result_bit1 = (curr_input[j].bit_1 & weight) | (curr_input[j].bit_0 & i_weight);
