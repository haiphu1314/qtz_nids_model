 ...
 * @ Modified time: 2024-07-10 19:40:05
 * @ Description:
 */
...
int main()
{
    ...
    for (int t = 0; t < len_typ; t++)
    {
        ...
        conv_layer *conv1 = create_conv_layer(1, 64, 3, 1, 1, 1, typ[t]);
        model = add_layer(model, CONV, "conv1", conv1);
        conv_layer *conv2 = create_conv_layer(64, 128, 3, 1, 1, 1, typ[t]);
        model = add_layer(model, CONV, "conv2", conv2);
        linear_layer *linear1 = create_linear_layer(128 * 7 * 7, 128, typ[t]);
        model = add_layer(model, LINEAR, "linear1", linear1);
        linear_layer *linear2 = create_linear_layer(128, 3, typ[t]);
